...
async def generate_exercise(
    request_data: ExerciseGenerationRequest,
    request: Request
):
...
async def generate_advanced_exercise(
    ...
    try:
        ...
        cache = request.app.state.redis_cache
        original_exercise_data = await cache.get(f"exercise:{request_data['previous_exercise_id']}")
        ...
        return GenerateExerciseResponse(
            exercise=exercise,
            message="Advanced exercise generated successfully"
        )
        ...
        raise HTTPException(status_code=500, detail="Advanced exercise generation failed")
