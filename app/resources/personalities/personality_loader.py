...
class PersonalityLoader:
    ...
    def _load_available_personalities(self) -> dict[str, str]:
        ...
        for file_path in self.personalities_dir.glob("*.md"):
            if file_path.name != "personality_loader.py":
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to load personality {personality_name}: {e}"
                    )

        logger.info(
            f"Loaded {len(personalities)} personality profiles",
    ...
    def apply_personality_to_prompt(
        self, base_prompt: str, personality: Optional[str] = None
    ) -> str:
...
"""
        ...
        logger.info(
            f"Applied personality '{personality or 'default'}' to system prompt"
        )
