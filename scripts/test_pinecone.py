...
class PineconeTestSuite:
    ...
    async def test_connection(self):
        ...
        try:
            ...
            self.log_test("Index Stats", True, f"{stats.total_vector_count:,} vectors, {stats.dimension} dimensions")
    ...
    async def test_vector_search(self):
        ...
        try:
            ...
            self.log_test("Embedding Generation", True, f"Generated {len(embedding)} dimensions")
            ...
            if results_count > 0:
                avg_score = sum(match.score for match in query_response.matches) / results_count
                self.log_test("Result Quality", True, f"Average relevance: {avg_score:.3f}")
                
                # Check metadata
                has_metadata = any(match.metadata for match in query_response.matches)
                self.log_test("Metadata Presence", has_metadata, "Metadata available" if has_metadata else "No metadata")
    ...
    async def test_concept_searches(self):
        ...
        for concept in test_concepts:
            try:
                ...
                results_count = len(query_response.matches)
                self.log_test(f"Concept: {concept}", True, f"{results_count} results found")
        ...
        if all_results:
            ...
            self.log_test("Concept Search Analysis", True, f"Average relevance: {avg_score:.3f}")
            ...
            self.log_test("Source Diversity", True, f"{len(books)} books, {len(subjects)} subjects")
    ...
    async def test_interest_integration(self):
        ...
        for query, description in interest_queries:
            try:
                ...
                results_count = len(query_response.matches)
                self.log_test(f"Interest: {description}", True, f"{results_count} contextual results")
    ...
    async def test_service_integration(self):
        ...
        try:
            ...
            self.log_test("Concept Context", True, f"Retrieved {len(context)} context chunks")
            
            # Test similar exercises
            similar_exercises = await service.find_similar_exercises(
                "test-concept-1",
                {"student_id": "test", "interests": ["sports"]}
            )
            self.log_test("Similar Exercises", True, f"Found {len(similar_exercises)} similar exercises")
            ...
            self.log_test("Remediation Examples", True, f"Found {len(remediation_examples)} remediation examples")
    ...
    async def test_enhanced_exercise_demo(self):
        ...
        try:
            ...
            query_text = f"{concept} basic examples explanations {' '.join(interests[:2])}"
            ...
            self.log_test("Context Retrieval", True, f"Retrieved {len(context_chunks)} context chunks")
            ...
            if context_chunks:
                ...
                self.log_test("Enhanced Prompt", True, f"Prompt enhanced with context from {context_chunks[0]['book']}")
                
                # Show benefit comparison
                print(f"\n    💡 Enhancement Benefits:")
                print(f"       📚 Educational accuracy from {context_chunks[0]['book']}")
                print(f"       🎯 Student interests: {', '.join(interests)}")
                print(f"       🔗 Relevant context with {context_chunks[0]['score']:.3f} relevance")
