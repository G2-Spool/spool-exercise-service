    ...
    async def test_environment_setup(self):
        ...
        for var_name, description in required_vars:
            ...
            if not value:
                ...
                self.log_test(f"Environment: {description}", False, f"{var_name} appears to be placeholder")
            else:
                self.log_test(f"Environment: {description}", True, f"***{value[-4:] if len(value) > 4 else '***'}")
    ...
    async def test_api_connections(self):
        ...
        try:
            ...
            if not api_key or api_key.startswith('test'):
                ...
                self.log_test("OpenAI API Connection", True, f"Response: {response.choices[0].message.content}")
        ...
        try:
            ...
            if not api_key:
                ...
                self.log_test("Pinecone API Connection", True, f"Found {len(indexes)} indexes")
        except Exception as e:
